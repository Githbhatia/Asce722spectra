...
class Root(Tk):
    def __init__(self):
        ...
        self.title("ASCE 7 Seismic Parameters")
        ...
        self.title_label = Label(text="ASCE7 Seismic Parameter Input").grid(row=rr,column=0, columnspan = 2,sticky="ew")

        rr+=1
        ASCEVerList=["ASCE7-16","ASCE7-22"]
        self.SelectedASCEVer=StringVar()
        self.SelectedASCEVer.set("ASCE7-22")
        self.label_ASCEVer = Label(text="ASCE Version").grid(row=rr,column=0)
        self.list_ASCEVer= OptionMenu(self, self.SelectedASCEVer,*ASCEVerList)
        self.list_ASCEVer.grid(row=rr,column=1,sticky="ew")
        
        rr+=1
        self.label_Title = Label(text="Title").grid(row=rr,column=0)
        self.entry_Title  = Entry()
        self.entry_Title.grid(row=rr,column=1,sticky="ew")
        self.entry_Title.insert(0,"My Site")

        rr+=1
        self.button = Button( text="Continue", bg='white', height=2, width=20, command=self.ASCEwhichVer).grid(row=rr,column=0)


    def ASCEwhichVer(self):
        if str(self.SelectedASCEVer.get())== "ASCE7-22":
            self.ASCE722()
        else:
            self.ASCE716()
    ...
    def ASCE716(self):
        ...
        newWindow.title("ASCE716")
        newWindow.grid_columnconfigure(1, minsize=200)
        
        newWindow.geometry('370x450')


        rr=5
        newWindow.title_SWVel = Label(text="Either enter Shear Wave Velocity or pick Site Class").grid(row=rr,column=0, columnspan = 2)
        rr+=1
        newWindow.title_SWVel2 = Label(text="(Shear Wave Velocity will be used when entered)").grid(row=rr,column=0, columnspan = 2)
        rr+=1
        newWindow.label_SWVel = Label(text="Shear Wave Velocity (ft/s)").grid(row=rr,column=0)
        newWindow.entry_SWVel  = Entry()
        newWindow.entry_SWVel.grid(row=rr,column=1,sticky="ew")
        newWindow.entry_SWVel.insert(0,"")

        SiteClassList=["A","B","C","D","E","F"]
        newWindow.SelectedSiteClass=StringVar()
        newWindow.SelectedSiteClass.set("D")
        rr+=1
        newWindow.label_SiteClass = Label(text="Site Class").grid(row=rr,column=0)
        newWindow.list_SiteClass = OptionMenu(newWindow, newWindow.SelectedSiteClass,*SiteClassList)
        newWindow.list_SiteClass.grid(row=rr,column=1,sticky="ew")

        RiskCategoryList=["I","II","III","IV"]
        newWindow.SelectedRiskCategory =StringVar()
        newWindow.SelectedRiskCategory.set("IV")
        rr+=1
        newWindow.label_RiskCategory = Label(text="Risk Category").grid(row=rr,column=0)
        newWindow.list_RiskCategory = OptionMenu(newWindow,newWindow.SelectedRiskCategory,*RiskCategoryList)
        newWindow.list_RiskCategory.grid(row=rr,column=1,sticky="ew")

        rr+=1
        newWindow.title_label = Label(text="Either provide Address or Lat/Long Pair (leave Address blank)").grid(row=rr,column=0, columnspan = 2)

        rr+=1
        newWindow.label_Address = Label(text="Address").grid(row=rr,column=0)
        newWindow.entry_Address = Entry()
        newWindow.entry_Address.grid(row=rr,column=1,sticky="ew")
        newWindow.entry_Address.insert(0,"")

        rr+=1  
        newWindow.label_Latitude = Label(text="Latitude").grid(row=rr,column=0)
        newWindow.entry_Latitude = Entry()
        newWindow.entry_Latitude.grid(row=rr,column=1,sticky="ew")
        newWindow.entry_Latitude.insert(0,"38")

        rr+=1  
        newWindow.label_Longitude = Label(text="Longitude").grid(row=rr,column=0)
        newWindow.entry_Longitude  = Entry()
        newWindow.entry_Longitude.grid(row=rr,column=1,sticky="ew")
        newWindow.entry_Longitude.insert(0,"-121")

        rr+=1
        newWindow.SaveJson=IntVar()
        newWindow.checkbutton_SaveJson=Checkbutton(newWindow, text="Save Json", variable=newWindow.SaveJson)
        newWindow.checkbutton_SaveJson.grid(row=rr,column=0,sticky="ew")

        newWindow.OpenMap=IntVar(value= 1)
        newWindow.checkbutton_OpenMap=Checkbutton(newWindow, text="Open Map", variable=newWindow.OpenMap)
        newWindow.checkbutton_OpenMap.grid(row=rr,column=1,sticky="ew")

        rr+=1
        newWindow.button = Button( text="Compute", bg='white', height=2, width=20, command=self.onclickASCE716).grid(row=rr,column=0)


    def ASCE722(self):
        self.geometry('370x450')
        rr=5
        self.title_SWVel = Label(text="Either enter Shear Wave Velocity or pick Site Class").grid(row=rr,column=0, columnspan = 2)
        rr+=1
        self.title_SWVel2 = Label(text="(Shear Wave Velocity will be used when entered)").grid(row=rr,column=0, columnspan = 2)
        rr+=1
        self.label_SWVel = Label(text="Shear Wave Velocity (ft/s)").grid(row=rr,column=0)
        self.entry_SWVel  = Entry()
        self.entry_SWVel.grid(row=rr,column=1,sticky="ew")
        ...
        self.list_SiteClass.grid(row=rr,column=1,sticky="ew")
        ...
        self.list_RiskCategory.grid(row=rr,column=1,sticky="ew")

        rr+=1
        self.title_label = Label(text="Either provide Address or Lat/Long Pair (leave Address blank)").grid(row=rr,column=0, columnspan = 2)

        rr+=1
        self.label_Address = Label(text="Address").grid(row=rr,column=0)
        self.entry_Address = Entry()
        self.entry_Address.grid(row=rr,column=1,sticky="ew")
        self.entry_Address.insert(0,"")

        rr+=1  
        self.label_Latitude = Label(text="Latitude").grid(row=rr,column=0)
        self.entry_Latitude = Entry()
        self.entry_Latitude.grid(row=rr,column=1,sticky="ew")
        self.entry_Latitude.insert(0,"38")

        rr+=1  
        self.label_Longitude = Label(text="Longitude").grid(row=rr,column=0)
        self.entry_Longitude  = Entry()
        self.entry_Longitude.grid(row=rr,column=1,sticky="ew")
        ...
        self.checkbutton_OpenMap.grid(row=rr,column=1,sticky="ew")

        rr+=1
        self.button = Button( text="Compute", bg='white', height=2, width=20, command=self.onclickASCE722).grid(row=rr,column=0)

        
    def onclickASCE716(self):
        if str(self.entry_SWVel.get()) != "":
            ...
            if shearwavevel==0:
                ...
                return
            shearwavevellimits = [('F',0.0),('E',600.0),('D',1200.0),('C',2500.0),('B',5000.0),('A',1000000.0)]
            index = 0
        ...
        url = 'https://earthquake.usgs.gov/ws/designmaps/asce7-16.json?latitude='+ lat + '&longitude=' + longt +'&riskCategory='+ riskct +'&siteClass=' + sitecl + '&title=Example'
        ...
        self.geometry('370x1150')     
        rdata = js.loads(response.read())
        if self.SaveJson.get() == 1:
            with open("ASCE716.json", "w") as write_file:
        ...
        self.title_label = Label(text="ASCE7-16 Seismic Parameter Output").grid(row=rr,column=0, columnspan = 2)
        index = 0
        for key, value in p.items():
            if index <= 29:
                rr+=1
                Label(self, text=str(key), relief = "sunken", width= 20).grid(column=0, row=rr)
                Label(self, text=str(value), relief = "sunken", width = 20).grid(column=1, row=rr)
            index += 1
 ...
 #           webbrowser.open("mymap.html")
    ...
    def onclickASCE722(self):
        ...
        rr= 20
        p = rdata["response"]["data"]
        self.title_label = Label(text="ASCE7-22 Seismic Parameter Output").grid(row=rr,column=0, columnspan = 2)
        index = 0
        for key, value in p.items():
            if index <= 11:
                rr+=1
